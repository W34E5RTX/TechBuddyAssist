...
const start = async () => { if (!process.env.MONGODB_URI) throw new Error('MONGODB_URI is required to start the API'); await mongoose.connect(process.env.MONGODB_URI, { serverSelectionTimeoutMS: 30000 }); app.listen(port, () => console.log(`TechBuddy Assist API listening on ${port}`)) }
start()
